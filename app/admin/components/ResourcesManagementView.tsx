    ...
    <div className="min-h-screen p-4 md:p-6 lg:p-8 animate-fade-in-up" style={{ background: "var(--background)" }}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={load} className="p-3 rounded-xl border-2 border-slate-200 hover:border-sky-400 text-slate-400 hover:text-sky-600 transition-all">
            <RefreshCw size={18}/>
          </button>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" {...{placeholder: t('ui.search_resource')}} value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm font-bold outline-none focus:border-sky-400 transition-all border-2" style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-primary)" }}/>
        </div>
      ...
      {/* Resources Grid */}
      {isLoading ? (
        <div className="flex items-center justify-center py-20"><Loader2 className="animate-spin text-sky-400" size={32}/></div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="p-5 bg-sky-50 rounded-3xl mb-4"><BookOpen size={40} className="text-sky-300"/></div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input type="url" value={newResource.url} onChange={e => setNewResource(p => ({ ...p, url: e.target.value }))}
                  placeholder="https://..."
                  className="w-full p-4 rounded-xl text-sm font-bold outline-none focus:border-sky-400 transition-all font-mono text-xs border-2" style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-primary)" }}/>
              </div>
