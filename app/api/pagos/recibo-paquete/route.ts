...
async function getCenterInfo() {
  try {
    ...
    if (data) return { nombre: data.nombre_centro || 'Jugando Aprendo', ruc: data.ruc || '', direccion: data.direccion || '', telefono: data.telefono || '', email: data.email || '' }
  } catch {}
  return { nombre: process.env.NEXT_PUBLIC_APP_NAME || 'Jugando Aprendo', ruc: '', direccion: '', telefono: '', email: '' }
}
