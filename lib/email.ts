...
function wrapHTML(content: string): string {
  ...
  <div class="card">
    <div class="header">
      <h1>${CENTRO}</h1>
      <p>Sistema de citas — Vanty ABA</p>
    </div>
...
export function buildEmailCita(accion: 'nueva' | 'actualizada' | 'cancelada', vars: CitaVars) {
  ...
  const html = wrapHTML(`
    ...
    ${accion === 'cancelada'
      ...
      : '<p>Podés ver todos los detalles en tu portal <strong>Vanty ABA</strong>.</p>'
    }
  `)
  return { subject, html }
}
...
export function buildEmailAdmin(accion: 'nueva' | 'actualizada' | 'cancelada', vars: CitaVars) {
  ...
  const html = wrapHTML(`
    ...
    <p>Revisá los detalles en el panel de administración de <strong>Vanty ABA</strong>.</p>
  `)
  return { subject, html }
}
